    ...
    try:
        ...
        if channel is not None:
            ...
            for item in channel.findall('item'):
                ...
                if title and link:
                    items_data.append({'title': title, 'description': description, 'link': link})
...
def display_feeds(feed_url,items,num_links_to_display):
    """this function displays the parsed feed."""
    
    print(f'\n---- Feed Entries from: {feed_url} ------')
    if not items:
        print("No items found in this feed.")
        return
    for item in items[:num_links_to_display]:
        print('\n--- Entry ----')
...
def multiple_url_feeds():
    ...
    for url in feed_urls:
        ...
        if xml_content:
            ...
            display_feeds(url,items,num_links)
